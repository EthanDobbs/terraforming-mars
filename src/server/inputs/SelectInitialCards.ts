import {AndOptions} from './basicInputs/AndOptions';
...
export class SelectInitialCards extends AndOptions {
  constructor(private player: IPlayer) {
    super('Select Initial Cards');
    let corporation: ICorporationCard;
    this.title = ' ';
    this.buttonLabel = 'Start';

    this.options.push(new SelectCard<ICorporationCard>(titles.SELECT_CORPORATION_TITLE, undefined, player.dealtCorporationCards).andThen((cards) => {
      if (cards.length !== 1) {
    ...
    if (player.game.gameOptions.preludeExtension) {
      this.options.push(new SelectCard(titles.SELECT_PRELUDE_TITLE, undefined, player.dealtPreludeCards, 2).andThen((preludeCards: Array<IProjectCard>) => {
    ...
    if (player.game.gameOptions.ceoExtension) {
      this.options.push(new SelectCard(titles.SELECT_CEO_TITLE, undefined, player.dealtCeoCards, 1).andThen((ceoCards: Array<ICeoCard>) => {
    ...
    this.options.push(new SelectMany<IProjectCard>(new CardSelection<IProjectCard>(titles.SELECT_PROJECTS_TITLE, undefined, player.dealtProjectCards), 0, 10).andThen((cards) => {
    ...
    this.cb = () => {
      ...
      player.game.playerHasPickedCorporationCard(player, corporation);
      return undefined;
    }
  }
}
