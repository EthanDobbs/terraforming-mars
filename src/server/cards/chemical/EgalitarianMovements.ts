...
export class EgalitarianMovements extends GlobalEvent implements IGlobalEvent {
  ...
  public resolve(game: IGame, turmoil: Turmoil) {
    game.getPlayersInGenerationOrder().forEach((player) => {
      const amount = player.tags.count(Tag.EARTH, 'raw') + turmoil.getPlayerInfluence(player);
      player.stock.deduct(Resource.MEGACREDITS, Math.min(amount, player.megaCredits), {log: true, from: this.name});
    });
  }
}
